...
function useSteamSearch() {
    ...
    const searchProfile = async (input)=>{
        ...
        setError(null);
        try {
            const response = await fetch(`/api/steam/resolve?input=${encodeURIComponent(input)}`);
            if (!response.ok) {
                const errorData = await response.json();
                // Convertimos cualquier error a string
                throw new Error(errorData.message || 'Error al buscar el perfil');
            }
            const data = await response.json();
            return data.steamId;
        } catch (err) {
            // Aseguramos que el error siempre sea un string
            const errorMessage = err instanceof Error ? err.message : 'Error desconocido al buscar el perfil';
            setError(errorMessage);
            throw errorMessage; // Lanzamos un string en lugar de un objeto Error
        } finally{
            setIsLoading(false);
        }
    };
...
function HomePage() {
    _s();
    const [steamId, setSteamId] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])('');
...
_s(HomePage, "tMScSD6MRvZxBeQ04geduW6Qrkg=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$app$2f$hooks$2f$useSteamPlayer$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useSteamPlayer"]
    ];
});
...
//# sourceMappingURL=src_app_dec55853._.js.map